...
- Plugin sidebar icons follow the same PNG+GIF-hover convention as category icons, but scoped per-plugin and resolved differently: drop `icon.png`/`icon.gif` into the plugin's own folder (under the resolved plugins dir, not exe-relative); `get_plugin_icon` (`commands/plugin_icons.rs`) reads them by discovery `dir` and returns data URLs (`usePluginIcon.ts` fetches/caches per plugin `dir`).
...
| `get_plugin_icon` | `commands/plugin_icons.rs` | Reads a plugin's `icon.png`/`icon.gif` (by discovery `dir`, validated as a single path segment) and returns each as a data URL. |
| `list_category_icons` | `commands/category_icons.rs` | Lists every `<name>.png`/`<name>.gif` pair in `<exe-dir>/assets/category-icons/` (resolved via `std::env::current_exe()`, not app-data) as data URLs. Both icon commands share a `read_as_data_url` helper in `commands/icon_util.rs`. |
