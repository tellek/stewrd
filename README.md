...
stewrd/
  ...
  plugins/                   plugin source (dev-time location — see §3 "where plugins actually live at runtime")
    ...
    notepad/                  Obsidian-style markdown notes: custom in-plugin CodeMirror editor (source/live-preview/reading modes), wikilinks/backlinks/callouts, per-note .md files via api.fs, background: true LLM-wiki harvester
...
| `notepad` | `true` | Full markdown editor bundling its own CodeMirror 6 instance (the shared `api.ui.CodeTextArea` has no extension seam and no markdown language) with a hand-rolled live-preview mark-hiding decoration; `markdown-it`-based Reading view with custom escaped renderer rules for wikilinks/embeds/callouts/checklists/footnotes; a plugin-maintained manifest (`index.json`) standing in for real directory listing of notes; `api.fs.listDir/getRootPath/deleteFile/renameFile` (new, generic, added alongside this plugin — see §6); `background: true` harvester spawning `claude` with a real `cwd` and bounded-retry file archiving instead of an infinite resubmit loop. |
