...
stewrd/
  ...
  src/                        React + TS host shell
    ...
    host/
      ...
      state/                   appStore.ts (zustand: plugins, activePluginId, statusLog, modals, toasts, categories, paletteId, customPalettes, palette (derived), sidebarCollapsed, sidebarItemsByPlugin, sidebarSelectedByPlugin, sidebarSubItemsExpanded, pluginsWithSidebarItems, view), hostSettings.ts (persists categories/theme/pluginsWithSidebarItems/sidebarSubItemsExpanded via storage_get/set under a reserved "__host__" id)
...
App.tsx
 ...
 │   ├─ expanded: SidebarCategory.tsx[] (grouped by each entry's resolved `category` - settings.json, falling back to plugin.json - resolved against appStore's categories, falling back to "Other") → SidebarPluginItem.tsx[] (indented, name + StatusIcon + optional plugin icon; click → setActivePlugin) + SidebarSubItems.tsx (further-indented, plugin-registered sub-items via api.sidebar.setItems - hidden when empty, not rendered in the collapsed rail)
...
- `hostSettings.ts`'s `saveHostSettings` is fire-and-forget (called from synchronous zustand actions), so `App.tsx` registers a Tauri `onCloseRequested` handler that calls `flushHostSettings()` (awaits every in-flight `storage_set` write tracked in a `pendingWrites` set) before letting the window actually close via `destroy()` (not `close()`, which would re-trigger `onCloseRequested`) - otherwise a setting changed right before quitting (e.g. toggling a sidebar sub-item's expand state, then immediately closing the app) can be lost mid-IPC-round-trip.
...
| `fs_read_text_file`, `fs_write_text_file`, `fs_list_dir`, `fs_get_root_path`, `fs_delete_file`, `fs_rename_file` | `commands/fs.rs` | Per-plugin scoped raw file access, atomic writes, path-escape rejection. `fs_list_dir` returns an empty list for a missing directory but propagates any other `read_dir` error; `fs_rename_file` creates the destination's parent directory as needed. |
